...
public class ResourceLocation extends AchieveREResponder {
    ...
    protected ACLMessage handleRequest(ACLMessage request) throws NotUnderstoodException, RefuseException {
        System.out.println(getAgent().getLocalName() + ":" + "Requesting Skill location\" "+request.getContent() + "\" request from " 
    ...
    protected ACLMessage prepareResultNotification(ACLMessage request, ACLMessage response) throws FailureException {
        System.out.println(getAgent().getLocalName() + ": Requesting Skill location\"" + request.getContent() + "\" requested from "
            + request.getSender().getLocalName());
